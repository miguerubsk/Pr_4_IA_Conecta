    ...
    public int jugada(Grid tablero, int conecta) {
        ...
        for (int i = 0; i < tablero.getFilas(); i++) {
            ...
            for (int j = 0; j < tablero.getColumnas(); j++) {
                if (tablero.getButton(i, j) != Conecta.VACIO) {
        ...
        for (int j = tablero.getColumnas() - 1; j >= 0; j--) {
            if (!tablero.fullColumn(j)) {
                if (tablero.topColumn(j) != Conecta.JUGADOR2) {
                    ganarHorz++;
                } else {
                    ganarHorz = 0;
                }
        ...
        for (int i = 0; i < tablero.getFilas(); i++) {
            if (tablero.getButton(i, posicion) != Conecta.VACIO) {
                if (tablero.getButton(i, posicion) != Conecta.JUGADOR2) {
                    ganarVert++;
                } else {
                    ganarVert = 0;
                }
        ...
        //Pintar Ficha
        return tablero.checkWin(tablero.setButton(posicion, Conecta.JUGADOR2), posicion, conecta);
    } // jugada          
